...
打开面板后，焦点默认留在「所有 Space」列表上（`↑`/`↓` 移动选中，`⏎` 跳转到选中的 Space）。聚焦列表时，按 `1`–`9` 可直接把选中行移到对应桌面的那一行（与行尾的「桌面 N」徽标对应），随后按 `⏎` 才真正跳转。按 `⌘F`（或点击列表上方的搜索条）将焦点移入筛选框：输入即按名称（或桌面编号，如 `3`）实时过滤列表，匹配片段加粗高亮，右侧显示匹配数量。筛选时 ↑/↓ 仍在过滤后的列表中移动，⏎ 跳转到选中的 Space，`Esc` 先清空筛选、再退出搜索条回到列表。搜索条是扁平圆角胶囊风格（放大镜图标 + 清除 ✕），刻意与「当前 Space」卡片里带边框的「Space 名称」输入框区分开。

不必先按 `⌘F`：列表聚焦时直接输入普通字符（字母、符号、空格；`1`–`9` 仍用于选中桌面行），搜索框会自动获得焦点并实时过滤——launcher 式输入即筛选。
...
`Ctrl+Shift+←` 一键跳回刚才激活的 Space——不用数桌面编号，Space 再多也没关系；连续按可沿着最近的 Space 激活历史一步步往回退。刻意不提供 `^⇧→`：桌面本身不会移动，再配一个前进键只会与系统自带的 `Ctrl+←` / `Ctrl+→` 导航造成错位——「回到上一个 Space」是单一、确定的方向。

这个跳转与点击列表跳转需要相同的配置：辅助功能权限 + 勾选「切换到桌面 1…9」快捷键。

设置界面（**Preferences…**）内置 **环境自检**：实时显示辅助功能授权状态、系统已启用的数字快捷键（如 `1, 2, 3, 4`），并有「请求权限…」按钮直接触发系统授权弹窗。如果「系统设置 → 隐私与安全性 → 辅助功能」里的开关怎么都开不上——这是 zip 安装残留的旧权限记录（旧签名条目）遮挡新授权的典型症状——点击「清理旧的辅助功能权限记录」即可清除 macOS 存储的该应用权限条目（即 `sudo tccutil reset Accessibility`，管理员密码由系统弹窗处理；应用内更新在替换新版前也会自动执行同样的清理）。清除后**必须重启应用**（macOS 会缓存当前进程的授权状态，仅重启后生效），应用会自动重启并重新弹出授权窗口，重新授权一次即可。
...
应用启动时会静默检查一次更新（每天最多一次）。打开 **Preferences…** → **更新** 区块，可随时手动「检查更新…」；发现新版本后点击「下载并重启」，应用会先自动清除旧的辅助功能权限记录（需要输入管理员密码），再退出、用新版本替换自身并自动重新启动，重启后自动重新请求辅助功能权限。

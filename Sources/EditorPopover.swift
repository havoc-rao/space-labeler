...
struct EditorPopover: View {
    ...
    /// Whether the full 13-color palette is expanded (default: one row).
    ...
    /// footprint, and the layout is fixed — the first row is always the 6
    /// light swatches + the expand chevron, so expanding only adds the dark
    /// row underneath. The two states never re-flow horizontally, no matter
    /// which color the current Space uses.
    private var colorPicker: some View {
        Grid(horizontalSpacing: 6, verticalSpacing: 6) {
            GridRow {
                ForEach(SpacePalette.lightColors, id: \.self) { hex in
                    swatchCell(hex)
                }
    ...
    /// the dark row and the chevron all line up on the same 7 columns.
    ...
    private func spaceRow(id: UInt64, desktop: Int) -> some View {
        ...
        return HStack(spacing: 9) {
            ...
            Text(highlightedName(label.name))
                .font(.system(size: 13))
                // Long names stay on one line: the tail truncates with
                // "…" (compressible before the right-side badges/delete
                // button), so the overflow never runs under the buttons.
                .lineLimit(1)
                .truncationMode(.tail)
                // Hovering the name shows the full untruncated text;
                // the rest of the row keeps its own jump/delete hint.
                .help(label.name)
            Spacer()
            if isCurrent {
